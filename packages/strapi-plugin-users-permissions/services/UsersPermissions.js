...
module.exports = {
  ...
  async updatePermissions() {
    ...
    if (!_.isEqual(permissionsFoundInDB.sort(), permissionsFoundInFiles.sort())) {
      ...
      const defaultEnable = [
        ['admincallback', 'auth', 'users-permissions', 'public'],
        ['adminregister', 'auth', 'users-permissions', 'public'],
        ['callback', 'auth', 'users-permissions', 'public'],
        ['connect', 'auth', 'users-permissions', null],
        ['forgotpassword', 'auth', 'users-permissions', 'public'],
        ['register', 'auth', 'users-permissions', 'public'],
        ['emailconfirmation', 'auth', 'users-permissions', 'public'],
        ['changepassword', 'auth', 'users-permissions', 'public'],
        ['init', null, 'userspermissions', null],
        ['me', 'user', 'users-permissions', null],
        ['autoreload', null, null, null],
      ];
      const getDefaultEnable = (permission, role) =>
        (defaultEnable[0] === null || permission.action === defaultEnable[0]) &&
        (defaultEnable[1] === null || permission.controller === defaultEnable[1]) &&
        (defaultEnable[2] === null || permission.type === defaultEnable[2]) &&
        (defaultEnable[3] === null || role.type === defaultEnable[3]);
